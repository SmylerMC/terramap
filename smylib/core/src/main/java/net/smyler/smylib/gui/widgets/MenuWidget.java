...
public class MenuWidget implements Widget {
        ...
        for (MenuEntry entry: this.entries) {
            ...
            if (entry.text != null) {
                ...
                if (subMenu != null && hovered && this.displayedSubMenu == null) {
                    ...
                    if (subY + subH > parent.getHeight()) {
                        subY = parent.getHeight() - subH - 1;
                    }
                    if (subX + subW > parent.getWidth()) {
                        subX -= subW + width + 1;
                    }
                    subMenu.z = this.z + 1;
                ...
                if (subMenu != null) {
                    this.font.draw(x + width - dw - padding, ty + padding, " >", c, false);
                }
                ty += lh;
            } else {
    ...
    public void hide(@Nullable WidgetContainer parent) {
        this.hideSubMenu(parent);
        if (parent != null) {
            parent.scheduleBeforeNextUpdate(() -> this.visible = false);
        } else {
            this.visible = false;
        }
