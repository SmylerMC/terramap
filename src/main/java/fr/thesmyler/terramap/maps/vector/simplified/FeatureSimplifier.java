...
/**
 * Simplify {@link VectorFeature} objects for cheaper rendering
 * 
 * @author SmylerMC
 *
 */
public class FeatureSimplifier {
    ...
    public VectorFeature simplify(VectorFeature feature, GeoBounds viewPort, double zoom, float distance) {
        if(!feature.getZoomRange().matches(zoom)) return null;
        if(!viewPort.intersects(feature.bounds())) return null;
        if(feature instanceof LineString) return this.simplifyLine((LineString) feature, viewPort, zoom, distance);
        //TODO Simplify more
        return feature;
    }
    
    //TODO Test
    private VectorFeature simplifyLine(LineString line, GeoBounds viewPort, double zoom, float distance) {
    ...
    private int countEdgeIntersections(GeoPoint point1, GeoPoint point2, GeoBounds bounds) {
