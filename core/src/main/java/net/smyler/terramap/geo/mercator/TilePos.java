...
public abstract class TilePos implements Mutable<TilePosImmutable>, Immutable<TilePosMutable> {
    ...
    public TilePos(int zoom, int x, int y) {
        if (zoom < 0 || zoom > WebMercatorUtil.MAX_ZOOM) {
            throw new InvalidTilePositionException("Invalid zoom level: " + zoom);
        }
        int mapSize = WebMercatorUtil.getDimensionsInTile(zoom);
        if (x < 0 || x >= mapSize) {
            throw new InvalidTilePositionException("Invalid coordinate: " + x + " for zoom level " + zoom);
        }
        if (y < 0 || y >= mapSize) {
            throw new InvalidTilePositionException("Invalid coordinate: " + y + " for zoom level " + zoom);
        }
        this.zoom = zoom;
        this.xPosition = x;
        this.yPosition = y;
