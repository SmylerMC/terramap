...
public final class TerramapUtil {
    ...
    public static boolean isServerEarthWorld(World world) {
        if (!(world.getWorldType() instanceof EarthWorldType)) {
            return false; // Is this a terra save?
        }
        if (!(world.getChunkProvider() instanceof ICubeProvider)) {
            return false; // Is a CC world (could be a different dimension)
        }
        ICubeProvider provider = (ICubeProvider) world.getChunkProvider();
        if (!(provider instanceof CubeProviderServer)) {
            return false; // Are we on server ?
        }
